...
func (s *Server) CRImportCheckpoint(
	...
	if systemCtx.SignaturePolicyPath != "" {
		return "", fmt.Errorf("namespaced signature policy defined for pods in namespace %s; signature validation is currently incompatible with restoring", sb.Metadata().Namespace)
	}
