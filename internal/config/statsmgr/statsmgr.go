...
type StatsManager interface {
	AddID(string, string)
	RemoveID(string)
	UpdateWithDiskStats([]*pb.ContainerStats) error
	Shutdown()
}
...
func updateWithDiskStats(criStats []*pb.ContainerStats, statsForIDFunc func(string) (uint64, uint64, error)) error {
	...
	for _, criStat := range criStats {
		...
		if criStat.WritableLayer == nil {
			criStat.WritableLayer = &pb.FilesystemUsage{}
		}
		...
		criStat.WritableLayer.UsedBytes = &pb.UInt64Value{Value: usedBytes}
		criStat.WritableLayer.InodesUsed = &pb.UInt64Value{Value: inodesUsed}
	}
