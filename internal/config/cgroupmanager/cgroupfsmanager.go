...
func (*CgroupfsManager) GetContainerCgroupPath(sbParent, containerID string) string {
	parent := defaultCgroupfsParent
	if sbParent != "" {
		parent = sbParent
	}
	return filepath.Join(parent, scopePrefix+"-"+containerID)
}
...
func (*CgroupfsManager) GetSandboxCgroupPath(sbParent, sbID string) (cgParent, cgPath string, err error) {
	...
	return sbParent, filepath.Join(sbParent, scopePrefix+"-"+sbID), nil
}
