...
func getSandboxCgroupPathForSystemd(sbParent, sbID, memorySubsystemPath, _ string) (cgParent, cgPath string, err error) {
	if sbParent == "" {
		return "", "", nil
	}
	...
	if err := verifyCgroupHasEnoughMemory(cgParent, memorySubsystemPath, "memory.limit_in_bytes"); err != nil {
		return "", "", err
	}
