...
func (c *ContainerServer) newConmonmon(r *oci.Runtime) (*conmonmon, error) {
	...
	ep, err := epoll.EpollCreate(&epoll.EpollConfig{
		OnWaitError: onWaitError,
	})
	if err != nil {
...
func (c *conmonmon) StopMonitoringConmon(ctr *oci.Container) {
	...
	fmt.Fprintf(os.Stderr, "removing conmon from watch\n")
	ci, found := c.conmons[ctr]
	if !found {
		fmt.Fprintf(os.Stderr, "couldn't find conmon to remove\n")
		return
	}
